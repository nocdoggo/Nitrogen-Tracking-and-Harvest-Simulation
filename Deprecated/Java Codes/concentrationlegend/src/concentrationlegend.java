...
public class concentrationlegend
{
  ...
  private void findmaxmin()
  {
    try
    {
      ...
      while ((linestr = textreader.readLine()) != null) {
        double curr_dep = Double.parseDouble((linestr.split("\\s+"))[5]);
        if (curr_dep > this.max)
          this.max = curr_dep;
        if ((curr_dep < this.min) && (curr_dep != 0.0D)) {
          this.min = curr_dep;
        }
  ...
  {
    concentrationlegend w = new concentrationlegend();
    w.folder = args[6];
    try
    {
      int width = 280; int height = 350;
      ...
      ig2.setFont(font);
      String message = "Concentration (Grains m-3)";
      ...
      ImageIO.write(bi, "PNG", new File("concentrationlegend.png"));

      FileInputStream fis = new FileInputStream("concentrationlegend.png");
      FileOutputStream fos = new FileOutputStream(w.folder + "\\concentrationlegend.png");
