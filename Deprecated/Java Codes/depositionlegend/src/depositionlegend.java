...
public class depositionlegend
{
  ...
  private void findmaxmin()
  {
    try
    {
      ...
      while ((linestr = textreader.readLine()) != null) {
        double curr_dep = Double.parseDouble((linestr.split("\\s+"))[4]);
  ...
  {
    depositionlegend w = new depositionlegend();
    //w.folder = "sudeep";
    w.folder = args[6];
    try
    {
      ...
      ig2.setFont(font);
      String message = "Deposition (grains/m2)";
      ...
      ImageIO.write(bi, "PNG", new File("depositionlegend.png"));

      FileInputStream fis = new FileInputStream("depositionlegend.png");
      FileOutputStream fos = new FileOutputStream(w.folder + "\\depositionlegend.png");
